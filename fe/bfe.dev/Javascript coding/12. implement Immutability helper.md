...
if we are to modify d to a new state, we could use [_.cloneDeep](https://lodash.com/docs/4.17.15#cloneDeep "null"), but it is not efficient because `state.a` is cloned while we don't need to change that.
...
Here comes the [Immutability Helper](https://reactjs.org/docs/update.html "null"), you are asked to implement your own Immutability Helper `update()`, which supports following features.
